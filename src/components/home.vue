<template>
  <div class="home">
    <header>
      <div class="header_bg">
        <div class="header_main" @click="toSelectObj()">
    ...
    <swiper class="swiper" loop auto dots-position="right" :interval="3000" :duration="500">
        <swiper-item class="swiper-demo-img" v-for="(item, index) in demo04_list" :key="index">
          <a href="#/bannerList"><img :src="item.imgSrc"></a>
        </swiper-item>
      </swiper>
      <ul class="main_item" ref="main_item">
        <li v-for="(item, index) in itemLi" :key="index" @click="goItems(index)">
          <img :src="item.img" />
          <p>{{item.text}}</p>
        </li>
      </ul>
    ...
    <footer class="home_footer">
      ...
      <a @click="goPage(2,'/wuliu')">
        <div><i class="iconfont icon-songhuohuoche"></i> </div>
        <p>物流</p>
      </a>
      <a @click="goPage(3,'/shopCar')">
        <div><i class="iconfont icon-cart"></i> </div>
        <p>购物车</p>
      </a>
      <a @click="goPage(4,'/myTaobao')">
...
export default {
  ...
  data() {
    return {
      demo04_list: [],
      itemLi: [
        { img: require("../assets/img/item01.png"), text: "天猫" },
        { img: require("../assets/img/item02.png"), text: "聚划算" },
        { img: require("../assets/img/item03.png"), text: "天猫国际" },
        { img: require("../assets/img/item04.png"), text: "外卖" },
        { img: require("../assets/img/item05.png"), text: "天猫超市" },
        { img: require("../assets/img/item06.png"), text: "充值中心" },
        { img: require("../assets/img/item07.png"), text: "飞猪旅行" },
        { img: require("../assets/img/item08.png"), text: "领金币" },
        { img: require("../assets/img/item09.png"), text: "拍卖" },
        { img: require("../assets/img/item10.png"), text: "分类" }
      ],
      //商品对应的id
      goodListId:[100,101,102,103],
      pdlist: [],
      navCloseType:true,
      timeH:"00",
      timeM:"00",
  ...
  methods: {
    ...
    upCallback: function() {
      ...
      .then(function (response) {
        ...
        self.pdlist = self.pdlist.concat(res);
        self.dbPages=response.data.dataPage;
        self.pageIndex++;
        self.mescroll.endByPage(response.data.result.length, response.data.dataPage)
      })
    ...
    goItems:function(i){
      var _this=this;
      if(i==0){
        _this.$router.push('tianmao');
      }
    },
    //跳转到不同的页面
    goPage:function(i,page){
      if(i==2||i==3||i==4){
        if(this.User){
          this.$router.push(page);
        }else{
          console.log('未登陆');
          this.$router.push({ path: '/login', query: { target: i }});
        }
      }else if(i==1){
    ...
    goMore(i){
      console.log(i);
      this.$router.push({'path':'/goodMores','query':{'id':i}})
    },
    toSelectObj:function(){
